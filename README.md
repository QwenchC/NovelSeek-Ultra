...
> 📸 **需要截图**：完整的应用主界面，底部导航栏全部可见，显示长篇小说项目列表页（或空状态）。横向展示两张：一张空状态、一张有项目卡片的状态。
...
> 📸 **需要截图**：设置页完整截图，显示各配置区块（语言切换、文本模型、图片引擎、备份）。

#### 2.2 配置文本模型（API）

在**文本模型配置**区块，点击 **「+ 添加」** 新建 API 配置：

- **API Base URL**：AI 文本服务地址（例如 `https://text.pollinations.ai` 免费可用）
- **API Key**：服务密钥（Pollinations 免费模式可留空）
- **模型名称**：填写你使用的模型 ID
...
> 📸 **需要截图**：设置页中"文本模型配置"区域的截图，显示已填写的一条配置（URL 和模型名可见，Key 打码），右侧有"测试连接"按钮。

#### 2.3 配置图片生成引擎

在**图片生成引擎**区块选择：

- **Pollinations**（默认）：云端免费，无需额外配置
- **ComfyUI**：连接本地局域网 ComfyUI（适合有本地 GPU 的用户）
...
> 📸 **需要截图**：新建项目的弹窗，显示"名称""类型""简介"输入框及"创建"按钮。
...
> 📸 **需要截图**：大纲生成页面，显示世界观/时间线/额外要求输入框、章节数量滑块，以及 AI 正在流式输出的大纲文本（文字滚动中的状态）。
...
> 📸 **需要截图**：保存大纲的对话框，显示各字段的勾选框（世界观/时间线/章节等），有预览文字和"确认"按钮。
...
> 📸 **需要截图**：点击"从大纲导入"后弹出的确认对话框，显示解析出的章节标题列表（如 3–5 个章节名）和"确认导入"按钮。
...
2. 点击 **「✨ 生成本章」** 按钮，AI 自动注入上下文（前章摘要、角色信息、世界观）并生成
3. 生成内容出现在**草稿区**，可直接编辑修改
4. 满意后复制或直接作为最终稿保存
...
> 📸 **需要截图**：长篇小说项目页的副本与弧线区域，显示至少 2 个副本（一个已展开显示弧线，一个收起），以及右侧或下方的"添加弧线"/"AI 生成"按钮。

#### 4.4 快捷调整弧线顺序

每条弧线右侧有 **↑↓ 按序号调序** 快捷按钮，拖动即可调整在副本内的先后顺序，后续章节序号自动顺延。

#### 4.5 章节生成（插入机制）

长篇章节生成与短篇相同，但额外支持：

- 在当前章节**前**或**后**插入新章节（编辑器顶栏操作）
- 插入后所有后续章节序号自动+1
...
> 📸 **需要截图**：角色导入弹窗，显示勾选列表（2–4 个角色名及简短描述），和"导入"按钮。
...
> 📸 **需要截图**：角色编辑对话框，显示已填写"一句话描述"输入框和"AI 生成"按钮，以及下方各字段（姓名/性别/身份/性格/背景/动机/形象）已被 AI 填充的状态。

#### 5.3 角色头像生成

在角色编辑框中，点击头像区域的 **「上传」** 可从相册选图，或点击 **「AI 生成」** 由图片引擎根据角色描述生成立绘。
...
- **长篇项目页**：点击顶栏右侧的 🗺️ 图标 → **境界信息**
- **项目页工具栏**：找到 **「修炼」** 按钮 → **修炼体系管理**

#### 6.1 添加境界

在修炼体系页，在顶部输入框填写境界名称与描述，点击 **「添加」**。

可在每个境界下添加子境界（如：炼气期 → 炼气一层、炼气二层……）。

![修炼体系编辑](docs/screenshots/06_cultivation.png)

> 📸 **需要截图**：修炼体系管理页，显示 2–3 个境界（其中一个已展开显示子境界），以及顶部添加表单。
...
> 📸 **需要截图**：境界信息对话框，显示"角色境界"Tab，有一个角色已展开，显示可选境界的列表，旁边有一个角色已被分配了境界（显示境界名称）。

---

### 7. 章节编辑器

章节编辑器是写作的核心页面，有三个 Tab：
...
> 📸 **需要截图**：章节编辑器页面，显示三个 Tab 标签（最终稿/草稿/插图），编辑器中有一定文字内容，顶部有章节标题，底部或角落有"生成本章"按钮。
...
在草稿区**长按并选中**一段文字，点击浮现的 **「改写」** 按钮，AI 对选中段落进行润色重写，结果替换原文。

#### 7.3 插入新章节

在编辑器顶部工具栏，点击 **「⊕ 前插」** 或 **「⊕ 后插」** 可在当前章节前/后插入空白章节，其后所有章节序号自动顺延。
...
> 📸 **需要截图**：编辑器"插图"Tab，显示已有一两段文字，其中一段高亮选中，旁边或下方有已生成的插图缩略图。
...
> 📸 **需要截图**：封面管理弹窗/页面，上方显示 1–2 张已生成的封面图（带左右翻页箭头），下方显示模型/风格/宽高配置区和"生成"按钮。
...
> 📸 **需要截图**：版本管理页，显示 3–5 条历史版本记录（带时间戳和备注说明），以及"保存当前版本"按钮和每条记录旁的"回退"按钮。
...
> 📸 **需要截图**：容器管理页，显示 1–2 个已创建的容器（显示容器名称、类型、开关状态），以及其中一个容器展开显示分块与值链（新旧条目可见）。
...
> 📸 **需要截图**：小说问答对话界面，显示一两轮对话（用户问题 + AI 回答气泡），AI 回答中有章节引用标注，底部有输入框和发送按钮。
...
> 📸 **需要截图**：智能体页面，显示用户在输入框发送了一条指令，下方显示智能体正在执行的工具调用链（如"正在生成大纲…""正在创建角色…"等步骤列表），底部有"停止"按钮。
...
> 📸 **需要截图**：听书页面，显示当前正在播放的章节名、进度条（滑块）、音色选择器、语速选择器，以及播放/暂停/上一章/下一章控制按钮。
...
> 📸 **需要截图**：导出页面，显示格式选择器（TXT/Markdown/PDF 三个 Chip），以及 PDF 选项的勾选列表，底部有"导出"按钮。
...
> 📸 **需要截图**：设置页中"数据备份"区域的截图，显示"导出备份"和"导入备份"两个按钮，以及简短的说明文字。
